...
const SMELINAR_PRICE_PER_M3 := 900.0
const SMELINAR_MIN_M3 := 0.1
const STORAGE_CAPACITY := 10.0
const CHOP_IN_M3 := 0.010
const CHOP_OUT_M3 := 0.015

var last_tab := ""
var smelinar_amount: SpinBox
var smelinar_stock_label: Label
var smelinar_value_label: Label
var smelinar_sell_button: Button
var storage_label: Label
var chop_button: Button
var chop_progress: ProgressBar
var chop_timer: Label
var chop_running := false
var chop_elapsed := 0.0
var chop_duration := 1.8
...
func _process(delta: float) -> void:
	...
	if main == null:
		return
	var tab := str(main.get("current_tab"))
...
func _build_left(main: Node) -> PanelContainer:
	var panel := _side_panel(main, 250)
	var box := _side_box(panel)
	var title := _label(main, "FIRMA", 22)
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title.add_theme_color_override("font_color", Color("#ffca42"))
	box.add_child(title)
	var sh := _label(main, "SKLAD", 17)
...
func _build_jobs(main: Node) -> PanelContainer:
	var panel := _side_panel(main, 270)
	var box := _side_box(panel)
	var title := _label(main, "ZAKÁZKY", 22)
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title.add_theme_color_override("font_color", Color("#ffca42"))
	box.add_child(title)
	var spacer := Control.new()
	...
	dm.add_child(dv)
	var dt := _label(main, "ŠMELINÁŘ", 18)
	...
	var row := HBoxContainer.new()
	dv.add_child(row)
	row.add_child(_label(main, "Prodat:", 13))
...
func _render_storage(main: Node) -> void:
	...
	var title := _label(main, "SKLAD", 28)
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title.add_theme_color_override("font_color", Color("#ffca42"))
	box.add_child(title)
	var state := _state(main)
	var used := _storage_used(state)
	var total := _label(main, "Celkem: %.3f / 10.0 m³" % used, 20)
	...
	var note := _label(main, "Všechny materiály sdílí maximální kapacitu 10 m³.", 15)
...
func _add_storage_card(main: Node, grid: GridContainer, title_text: String, amount: float) -> void:
	...
	var box := VBoxContainer.new()
	margin.add_child(box)
	var t := _label(main, title_text, 18)
	t.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	t.add_theme_color_override("font_color", Color("#ffca42"))
	box.add_child(t)
	var a := _label(main, "%.3f m³" % amount, 24)
...
func _sell_to_smelinar() -> void:
	...
	if main == null or not is_instance_valid(smelinar_amount):
		return
	var state := _state(main)
	var available := float(state.get("split_m3", 0.0))
	var amount := snappedf(float(smelinar_amount.value), 0.1)
...
func _refresh_smelinar(main: Node) -> void:
	if not is_instance_valid(smelinar_amount):
		return
	var available := maxf(0.0, float(_state(main).get("split_m3", 0.0)))
	var sellable := floor(available * 10.0 + 0.0001) / 10.0
...
func _refresh_storage_label(main: Node) -> void:
	if not is_instance_valid(storage_label):
		return
	var used := _storage_used(_state(main))
...
func _side_box(panel: PanelContainer) -> VBoxContainer:
	return (panel.get_child(0) as MarginContainer).get_child(0) as VBoxContainer

func _load_player_texture(main: Node) -> Texture2D:
	var equipped := str(_state(main).get("equipped_axe", "wooden"))
	var path := "res://assets/characters/player_sharp_1.png" if equipped == "sharpened" else "res://assets/characters/player_wood_1.png"
...
func _load_company_background() -> Texture2D:
	var candidates := ["res://assets/backgrounds/company_yard.png", "res://assets/backgrounds/sluncem_zalitý_dvůr_venkovské_chalupy.png"]
...
func _style(main: Node, bg: String, border: String, radius: int, width: int) -> StyleBoxFlat:
	...
	return value as StyleBoxFlat if value is StyleBoxFlat else StyleBoxFlat.new()
