...
const CATEGORIES: Array[String] = ["SEKERY", "PILY", "DOPRAVNÍ PROSTŘEDKY", "PRODEJ DŘEVA"]
const LOG_PRICE_PER_M3: float = 1200.0
const BLOCK_PRICE_MIN: int = 1100
const BLOCK_PRICE_MAX: int = 1250
const SALE_STEP: float = 0.1
...
var block_price_per_m3: float = 1100.0
var log_sale_amount: float = 0.1
var block_sale_amount: float = 0.1

func _ready() -> void:
	load_inventory()
	randomize()
	block_price_per_m3 = float(randi_range(BLOCK_PRICE_MIN, BLOCK_PRICE_MAX))
...
func _render_category(main: Node, host: VBoxContainer) -> void:
	if current_category == "PRODEJ DŘEVA":
		_render_wood_sales(main, host)
		return
...
func _render_wood_sales(main: Node, host: VBoxContainer) -> void:
	var state := _main_state(main)
	var used := _storage_used(state)
	var storage := _make_label(main, "Sklad: %.2f / 10.00 m³" % used, 16)
	...
	grid.add_theme_constant_override("h_separation", 12)
	host.add_child(grid)
	_add_wood_sale_card(main, grid, "logs_m3", "KLÁDY", LOG_PRICE_PER_M3, log_sale_amount)
	_add_wood_sale_card(main, grid, "roundwood_m3", "ŠPALKY", block_price_per_m3, block_sale_amount)

func _add_wood_sale_card(main: Node, grid: GridContainer, key: String, title_text: String, price: float, amount: float) -> void:
	var state := _main_state(main)
	var owned := float(state.get(key, 0.0))
	...
	box.add_child(_make_label(main, "Výkup: %.0f Kč / m³" % price, 15))
	if key == "roundwood_m3":
		box.add_child(_make_label(main, "Cena se pohybuje 1100–1250 Kč / m³.", 13))
	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 6)
	box.add_child(row)
	var minus := Button.new(); minus.text = "− 0,1"; minus.pressed.connect(_change_sale_amount.bind(key, -SALE_STEP)); row.add_child(minus)
	var amount_label := _make_label(main, "%.1f m³" % amount, 17); amount_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL; amount_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; row.add_child(amount_label)
	var plus := Button.new(); plus.text = "+ 0,1"; plus.pressed.connect(_change_sale_amount.bind(key, SALE_STEP)); row.add_child(plus)
	var payout := amount * price
	var sell := Button.new()
	sell.text = "PRODAT %.1f m³ ZA %.0f Kč" % [amount, payout]
	sell.custom_minimum_size.y = 40
	sell.disabled = owned + 0.0001 < amount or amount < SALE_STEP
	sell.pressed.connect(_sell_wood.bind(key, amount, price))
	box.add_child(sell)

func _change_sale_amount(key: String, delta: float) -> void:
	if key == "logs_m3":
		log_sale_amount = maxf(SALE_STEP, snappedf(log_sale_amount + delta, SALE_STEP))
	else:
		block_sale_amount = maxf(SALE_STEP, snappedf(block_sale_amount + delta, SALE_STEP))
	_refresh_category()

func _sell_wood(key: String, amount: float, price: float) -> void:
	var main := get_tree().current_scene
	if main == null:
		return
	var state := _main_state(main)
	var owned := float(state.get(key, 0.0))
	if amount < SALE_STEP or owned + 0.0001 < amount:
		return
	state[key] = maxf(0.0, owned - amount)
	state["money"] = float(state.get("money", 0.0)) + amount * price
...
func _add_item_card(main: Node, grid: GridContainer, item_id: String, item: Dictionary) -> void:
	...
	grid.add_child(panel)
	var margin := MarginContainer.new()
	margin.add_theme_constant_override("margin_left", 14); margin.add_theme_constant_override("margin_right", 14); margin.add_theme_constant_override("margin_top", 12); margin.add_theme_constant_override("margin_bottom", 12)
	panel.add_child(margin)
	...
	var name_label := _make_label(main, str(item["name"]), 20); name_label.add_theme_color_override("font_color", Color("#ffca42")); info.add_child(name_label)
	info.add_child(_make_label(main, str(item["desc"]), 14))
	var price: int = int(item["price"]); var owned: int = _owned_count(main, item_id); var price_text := "Startovní výbava" if price <= 0 else "%d Kč" % price
	...
	if item_id == "wooden_axe":
		action.text = "VYBAVIT"; action.pressed.connect(_equip_axe.bind("wooden"))
	elif item_id == "sharpened_axe" and owned > 0:
		action.text = "VYBAVIT"; action.pressed.connect(_equip_axe.bind("sharpened"))
	else:
		action.text = "KOUPIT ZA %d Kč" % price; action.disabled = float(_main_state(main).get("money", 0.0)) < float(price); action.pressed.connect(_buy_item.bind(item_id))
